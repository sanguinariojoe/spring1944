...
local function GenerateUnitGraphics(uid, udid, getAuras)
    ...
    local transid = GetUnitTransporter(uid)
    local isNotTurret = transid and not ud.customParams.child
    local isNotGarrisoned = transid and not UnitDefs[GetUnitDefID(transid)].customParams.garrison
    if not Spring.ValidUnitID(uid) or (isNotTurret and isNotGarrisoned) then
        return
    end
    ...
    if ud.customParams.child or (transid and not isNotGarrisoned) then
        display = true
    end
